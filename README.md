...
You will notice that, by default, an `rss.xml` RSS feed is generated by the `syte build` command. In order to populate feed items, your pages will need to have `date` and `title` frontmatter items.
...
There is currently no way to disable the RSS feed. You can remove the RSS XML file automatically with this build command:
```
syte build && rm build/rss.xml
```

Also, the feed items do not yet contain the contents of the posts. This is planned.